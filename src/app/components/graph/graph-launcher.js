      ...
      link: function (scope, _element) {
        ...
        scope.showExpanded = function () {
          ...
          if (node && node.resource_type == "source") {
            nodes = graph.showFullGraph(
              "source:" + node.source_name + "." + node.name
            );
          ...
          trackingService.track_graph_interaction(
            "show-expanded",
            nodes.length
          );
        };
        ...
        scope.onLauncherClick = function () {
          ...
          if (node) {
            ...
            scope.showExpanded();
            var spec = selectorService.selectSource($state.params.source, {
              children: true,
            });
            // eslint-disable-next-line angular/timeout-service -- TODO
            setTimeout(function () {
        ...
        scope.$watch(
          ...
          function (_nv, _ov) {
            ...
            var clean_nodes = selectorService.selectNodes(
              dag,
              all_nodes,
              clean_selector
            );
            var dirty_nodes = selectorService.selectNodes(
              dag,
              all_nodes,
              dirty_selector
            );

            var nodes_to_remove = _.difference(
              clean_nodes.selected,
              dirty_nodes.selected
            );
