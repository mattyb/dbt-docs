...
angular.module("dbt").directive("docsSearch", [
  "$sce",
  "project",
  function ($sce, projectService) {
    return {
      scope: {
        query: "=",
        results: "=",
      ...
      link: function (scope) {
        ...
        scope.$watchGroup(watchExpressions, function () {
          scope.results = filterResults(
            projectService.search(scope.query),
            scope.checkboxStatus
          );
        });
        ...
        scope.shorten = function (text) {
          if (
            ...
            let startIndex =
              indexOfInstance - 75 < 0 ? 0 : indexOfInstance - 75;
            let endIndex =
              indexOfInstance + 75 > modified.length
                ? modified.length
                : indexOfInstance + 75;
            let shortened =
              "..." + modified.substring(startIndex, endIndex) + "...";
            return shortened;
          }
          return text;
        };
        ...
        scope.limitColumns = function (id) {
          return scope.limit_columns[id] !== undefined
            ? scope.limit_columns[id]
            : 3;
        };
