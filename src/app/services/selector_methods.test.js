...
test("Test splitting specs", () => {
  expect(selectors.splitSpecs("a b c")).toStrictEqual(["a", "b", "c"]);

  expect(selectors.splitSpecs("a b  ")).toStrictEqual(["a", "b"]);

  expect(selectors.splitSpecs("a   c")).toStrictEqual(["a", "c"]);

  expect(selectors.splitSpecs(" b  c")).toStrictEqual(["b", "c"]);
});
...
test("Test parsing specs (fqn)", () => {
  expect(selectors.parseSpec("a")).toStrictEqual({
    select_at: false,
...
test("Test parsing specs (fqn with parents and children)", () => {
  expect(selectors.parseSpec("+a+5")).toStrictEqual({
    select_at: false,
...
test("Test parsing specs (explicit fqn)", () => {
  expect(selectors.parseSpec("@fqn:a")).toStrictEqual({
    select_at: true,
    select_children: false,
    select_parents: false,
    parents_depth: null,
...
test("Test parsing specs (explicit tag)", () => {
  expect(selectors.parseSpec("@tag:a")).toStrictEqual({
    select_at: true,
    select_children: false,
    select_parents: false,
    parents_depth: null,
...
test("Test parsing specs (explicit source)", () => {
  expect(selectors.parseSpec("source:a+1")).toStrictEqual({
    select_at: false,
    select_children: true,
    select_parents: false,
    parents_depth: null,
...
test("Test parsing specs (explicit source.table)", () => {
  expect(selectors.parseSpec("source:a.b+")).toStrictEqual({
    select_at: false,
    select_children: true,
    select_parents: false,
    parents_depth: null,
...
test("Test parsing specs (explicit exposure)", () => {
  expect(selectors.parseSpec("+exposure:a")).toStrictEqual({
    select_at: false,
    select_children: false,
    select_parents: true,
    parents_depth: null,
...
test("Test parsing specs (explicit metric)", () => {
  expect(selectors.parseSpec("+metric:a")).toStrictEqual({
    select_at: false,
    select_children: false,
    select_parents: true,
    parents_depth: null,
...
test("Test parsing specs (scoped fqn)", () => {
  expect(selectors.parseSpec("a.b.c+")).toStrictEqual({
    select_at: false,
    select_children: true,
    select_parents: false,
    parents_depth: null,
...
test("Test set based selectors", () => {
  expect(selectors.parseSpecs("a,b c")).toStrictEqual([
    {
      method: "intersect",
      selectors: [
        {
...
test("Test applying selector specs", () => {
  ...
  expect(selectors.applySpec("a b c", (el) => lookup[el.raw])).toStrictEqual({
    matched: [1, 2, 3],
    selected: [1, 2, 3],
  });
});
...
test("Test parsing specs", () => {
  expect(selectors.buildSpec("a,tag:b+ c", "d", 1)).toStrictEqual({
    include: [
      {
        method: "intersect",
        selectors: [
