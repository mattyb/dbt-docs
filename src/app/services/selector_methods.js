...
function applySpec(selectorString, getMatchingNodes) {
  ...
  _.each(selectorSets, function (selectorList) {
    var setMethod =
      selectorList.method == "intersect" ? _.intersection : _.union;
...
function selectNodes(dag, pristine, selected_spec) {
  ...
  _.each(nodes_to_include, function (node_id) {
    ...
    var matched_package = _.includes(
      selected_spec.packages,
      node.data.package_name
    );
    var matched_tags =
      _.intersection(selected_spec.tags, node.data.tags).length > 0;
    var matched_untagged =
      _.includes(selected_spec.tags, null) && node.data.tags.length == 0;
    var matched_types = _.includes(
      selected_spec.resource_types,
      node.data.resource_type
    );

    if (
      !matched_package ||
      (!matched_tags && !matched_untagged) ||
      !matched_types
    ) {
