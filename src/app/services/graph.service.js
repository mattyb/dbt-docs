  ...
  function (
    ...
    var layouts = {
      ...
      top_down: {
        ...
        positions: function (node) {
          ...
          var parents = _.sortBy(
            selectorGraph.ancestorNodes(dag, primary_node_id, 1)
          );
          var children = _.sortBy(
            selectorGraph.descendentNodes(dag, primary_node_id, 1)
          );
    ...
    function getNodeVertPosition(primary_node, parents, children, this_node) {
      ...
      if (primary_node == this_node) {
        ...
        config = {
          set: parents,
          index: _.indexOf(parents, this_node),
          factor: -1,
          type: "parent",
        };
        ...
        config = {
          set: children,
          index: _.indexOf(children, this_node),
          factor: 1,
          type: "child",
        };
      } else {
    ...
    function setNodes(node_ids, highlight, classes) {
      var nodes = _.map(node_ids, function (id) {
        return service.graph.pristine.nodes[id];
      });
      var edges = [];
      _.flatten(
        _.each(node_ids, function (id) {
          var node_edges = service.graph.pristine.edges[id];
          _.each(node_edges, function (edge) {
            if (
              _.includes(node_ids, edge.data.target) &&
              _.includes(node_ids, edge.data.source)
            ) {
              edges.push(edge);
            }
          });
      ...
      service.graph.elements = _.filter(elements, function (e) {
        return e.data.display == "element";
      });
      return node_ids;
    }
    ...
    projectService.ready(function (project) {
      ...
      _.each(service.manifest.parent_map, function (parents, child) {
        _.each(parents, function (parent) {
          ...
          if (
            !_.includes(
              ["model", "source", "seed", "snapshot", "metric"],
      ...
      service.graph.elements = _.flatten(
        _.values(service.graph.pristine.nodes).concat(
          _.values(service.graph.pristine.edges)
        )
      );
      setNodes(dag.nodes());
    });
