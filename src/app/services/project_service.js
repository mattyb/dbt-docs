...
angular.module("dbt").factory("project", [
  "$q",
  "$http",
  function ($q, $http) {
    var TARGET_PATH = "";
    ...
    function incorporate_catalog(manifest, catalog) {
      ...
      _.each(manifest.nodes, function (node, node_id) {
        ...
        var new_columns = match_dict_keys(
    ...
    function loadFile(label, path) {
      return $http({
        ...
        function onError(e) {
          console.error(e);
          alert(
            "dbt Docs was unable to load the " +
              label +
              " file at path: \n  " +
              path +
              "\n\nError: " +
    ...
    service.loadProject = function () {
      ...
      $q.all(promises).then(function (files) {
        ...
        var macros = clean_project_macros(
          service.files.manifest.macros,
          adapter
        );
        service.files.manifest.macros = macros;

        var project = incorporate_catalog(
        ...
        _.each(tests, function (test) {
          ...
          if (test.test_metadata.namespace) {
            test_name =
              test.test_metadata.namespace + "." + test.test_metadata.name;
          } else {
        ...
        service.project.searchable = _.filter(
    ...
    service.updateSelectedInTree = function (select, subtrees) {
      ...
      _.each(subtrees, function (subtree) {
        if (subtree.node && subtree.node.unique_id == select) {
          ...
          var child_active = service.updateSelectedInTree(
            select,
            subtree.items
          );
          if (child_active) {
    ...
    service.caseColumn = function (col) {
      if (
        service.project.metadata.adapter_type == "snowflake" &&
        col.toUpperCase() == col
      ) {
