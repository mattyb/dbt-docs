...
test("Test FQN Matching glob", () => {
  expect(
    matcher.isFQNMatch(["my_package", "dir", "model"], "my_package.*")
  ).toStrictEqual(true);
});

test("Test FQN Matching all parts", () => {
  expect(
    matcher.isFQNMatch(["my_package", "dir", "model"], "my_package.dir.model")
  ).toStrictEqual(true);
});
...
test("Test FQN Matching bare path", () => {
  expect(
    matcher.isFQNMatch(["my_package", "dir", "model"], "my_package.dir")
  ).toStrictEqual(true);
});
...
test("Test FQN Matching glob all", () => {
  expect(matcher.isFQNMatch(["my_package", "dir", "model"], "*")).toStrictEqual(
    true
  );
});
...
test("Test FQN Matching direct model name", () => {
  expect(
    matcher.isFQNMatch(["my_package", "dir", "model"], "model")
  ).toStrictEqual(true);
});

test("Test FQN Matching direct model name with dots", () => {
  expect(
    matcher.isFQNMatch(["my_package", "dir", "ns1.ns2.model"], "ns1.ns2.model")
  ).toStrictEqual(true);
});

test("Test FQN Matching non match package", () => {
  expect(
    matcher.isFQNMatch(["my_package", "dir", "model"], "other_package")
  ).toStrictEqual(false);
});

test("Test FQN Matching non match path", () => {
  expect(
    matcher.isFQNMatch(["my_package", "dir", "model"], "my_package.other_dir")
  ).toStrictEqual(false);
});
...
test("Test getting nodes by FQN", () => {
  ...
  expect(matchByFQN("dir.model")).toStrictEqual([1]);

  expect(matchByFQN("dir.*")).toStrictEqual([1, 2, 3]);

  expect(matchByFQN("model")).toStrictEqual([1]);

  expect(matchByFQN("other_package")).toStrictEqual([3]);

  expect(matchByFQN("other_package.*")).toStrictEqual([3]);

  expect(matchByFQN("*")).toStrictEqual([1, 2, 3]);
});
...
test("Test getting nodes by Tag", () => {
  ...
  expect(matchByTag("nightly")).toStrictEqual([3]);

  expect(matchByTag("daily")).toStrictEqual([1, 2, 3]);
});
...
test("Test getting nodes by Source", () => {
  ...
  expect(matchBySource("events")).toStrictEqual([4, 5]);

  expect(matchBySource("events.my_event")).toStrictEqual([4]);

  expect(matchBySource("badsource")).toStrictEqual([]);

  expect(matchBySource("*")).toStrictEqual([4, 5, 6]);
...
test("Test getting nodes by path", () => {
  ...
  expect(matchByPath("models/dir/model.sql")).toStrictEqual([1]);

  expect(matchByPath("models")).toStrictEqual([1, 2, 3, 4, 5, 6]);

  expect(matchByPath("models/")).toStrictEqual([1, 2, 3, 4, 5, 6]);

  expect(matchByPath("models/dir")).toStrictEqual([1, 2, 3]);

  expect(matchByPath("models/dir/")).toStrictEqual([1, 2, 3]);

  expect(matchByPath("mod")).toStrictEqual([]);

  expect(matchByPath("badpath/")).toStrictEqual([]);
});
...
test("Test getting nodes by package", () => {
  ...
  expect(matchByPackage("my_package")).toStrictEqual([1, 2, 4, 5, 6, 7, 8]);

  expect(matchByPackage("other_package")).toStrictEqual([3]);

  expect(matchByPackage("")).toStrictEqual([]);

  expect(matchByPackage("badpackagename")).toStrictEqual([]);
});
...
test("Test getting nodes by config", () => {
  ...
  expect(matchByConfig({ config: "madeup", value: "notreal" })).toStrictEqual(
    []
  );

  expect(matchByConfig({ config: "severity", value: "error" })).toStrictEqual([
    7,
  ]);

  expect(matchByConfig({ config: "severity", value: "warn" })).toStrictEqual([
    8,
  ]);
});
...
test("Test getting nodes by test name", () => {
  ...
  expect(matchByTestName("unique")).toStrictEqual([7]);

  expect(matchByTestName("bad")).toStrictEqual([]);
});
...
test("Test getting nodes by test type", () => {
  ...
  expect(matchByTestType("data")).toStrictEqual([8]);

  expect(matchByTestType("singular")).toStrictEqual([8]);

  expect(matchByTestType("schema")).toStrictEqual([7]);

  expect(matchByTestType("generic")).toStrictEqual([7]);

  expect(matchByTestType("invalid")).toStrictEqual([]);
});
